...
-- Helper function to check if current user is super admin (SECURITY DEFINER to bypass RLS)
...
-- Helper function to get the current user's ID (SECURITY DEFINER to bypass RLS)
...
-- Helper function to get client IDs where current user is admin (SECURITY DEFINER to bypass RLS)
CREATE OR REPLACE FUNCTION get_my_admin_client_ids()
RETURNS SETOF uuid
LANGUAGE sql
SECURITY DEFINER
STABLE
...
-- Helper function to get client IDs where current user has participated or organized
CREATE OR REPLACE FUNCTION get_my_client_ids()
RETURNS SETOF uuid
LANGUAGE sql
SECURITY DEFINER
STABLE
AS $$
  SELECT DISTINCT client_id FROM (
    -- Clients where I participated
    SELECT p.client_id FROM participations p
    JOIN users u ON u.id = p.user_id
    WHERE u.auth_user_id = auth.uid()
    UNION
    -- Clients where I organized
    SELECT w.client_id FROM workshops w
    JOIN users u ON u.id = w.organizer
    WHERE u.auth_user_id = auth.uid()
    UNION
    -- Clients where I co-organized
    SELECT w.client_id FROM workshops w
    JOIN workshop_co_organizers wco ON wco.workshop_id = w.id
    JOIN users u ON u.id = wco.user_id
    WHERE u.auth_user_id = auth.uid()
    UNION
    -- Clients where I am admin
    SELECT ca.client_id FROM client_admins ca
    JOIN users u ON u.id = ca.user_id
    WHERE u.auth_user_id = auth.uid()
  ) AS all_clients;
$$;

-- Helper function to check if a user belongs to any of my clients
CREATE OR REPLACE FUNCTION user_is_in_my_clients(target_user_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
AS $$
  SELECT EXISTS (
    -- Target user participated in my client
    SELECT 1 FROM participations p
    WHERE p.user_id = target_user_id
    AND p.client_id IN (SELECT get_my_client_ids())
    UNION
    -- Target user organized in my client
    SELECT 1 FROM workshops w
    WHERE w.organizer = target_user_id
    AND w.client_id IN (SELECT get_my_client_ids())
    UNION
    -- Target user co-organized in my client
    SELECT 1 FROM workshop_co_organizers wco
    JOIN workshops w ON w.id = wco.workshop_id
    WHERE wco.user_id = target_user_id
    AND w.client_id IN (SELECT get_my_client_ids())
  );
$$;

-- Helper function to check if target user belongs to a client where I am admin
CREATE OR REPLACE FUNCTION user_is_in_my_admin_clients(target_user_id uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
STABLE
AS $$
  SELECT EXISTS (
    -- Target user participated in my admin client
    SELECT 1 FROM participations p
    WHERE p.user_id = target_user_id
    AND p.client_id IN (SELECT get_my_admin_client_ids())
    UNION
    -- Target user organized in my admin client
    SELECT 1 FROM workshops w
    WHERE w.organizer = target_user_id
    AND w.client_id IN (SELECT get_my_admin_client_ids())
  );
$$;

-- ============================================================================
-- USERS POLICIES (without recursion)
-- ============================================================================

-- Public can read users for login flow (checking if email exists)
...
-- Users can view other users from the same client (for co-organizer selection)
CREATE POLICY "users_select_same_client"
  ON users FOR SELECT
  TO authenticated
  USING (user_is_in_my_clients(id));

-- Client admins can manage all users in their client
CREATE POLICY "users_all_client_admin"
  ON users FOR ALL
  TO authenticated
  USING (user_is_in_my_admin_clients(id))
  WITH CHECK (user_is_in_my_admin_clients(id));

-- Super admins can view all users (but NOT modify - their role is managing clients/admins)
